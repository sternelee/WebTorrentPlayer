...
pub fn install(mpv_ctx: NonNull<mpv_handle>, ns_window_ptr: i64) -> Result<(), String> {
...

    {
        ...
        if let Some(stale) = existing.take() {
            eprintln!("[torplay::mpv_mac] WARNING: stale embed present at install; leaking RenderContext to avoid use-after-free");
    ...
    unsafe {
        ...
        eprintln!(
            "[torplay::mpv_mac] contentView bounds: {}x{}",
        ...
        *slot().lock().map_err(|e| format!("slot lock: {}", e))? = Some(Embed {
        ...
        eprintln!("[torplay::mpv_mac] installed");
    }
    Ok(())
}
...
pub fn render_now() -> Result<(), String> {
    ...
    unsafe {
        ...
        if LAST_SURFACE.swap(packed, Ordering::Relaxed) != packed {
            eprintln!(
                "[torplay::mpv_mac] render surface {}x{} px (bounds {}x{} pt)",
                w,
                h,
        ...
        let render = embed.render.lock().map_err(|e| format!("render lock: {}", e))?;
        render
            .render::<()>(0, w, h, true)
...
pub fn uninstall() -> Result<(), String> {
    ...
    eprintln!("[torplay::mpv_mac] uninstalled");
    Ok(())
}
